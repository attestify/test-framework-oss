...
pub fn nape_testing_dir() -> PathBuf {
    ...
    if !home_dir.exists() {
        fs::create_dir_all(&home_dir).expect("Could not create the NAPE Testing Root directory.");
    }
...
pub fn remove_nape_testing_dir() {
    ...
    if nape_testing_dir.exists() {
        fs::remove_dir_all(&nape_testing_dir).expect(
            format!(
                "Could not remove the NAPE Testing Root directory '{}'.",
                nape_testing_dir.display()
            )
            .as_str(),
        );
    }
    ...
    if !nape_test_root.exists() {
        if nape_test_root.extension().is_some() {
            ...
            if let Some(parent) = nape_test_root.parent() {
                ...
                fs::File::create(&nape_test_root).expect(
                    format!("Could not create the file '{}'.", nape_test_root.display()).as_str(),
                );
            }
        } else {
            // It's a directory
            fs::create_dir_all(&nape_test_root).expect(
                format!(
                    "Could not create the directory '{}'.",
                    nape_test_root.display()
                )
                .as_str(),
            );
        }
...
pub fn generate_nape_testing_file(subpath: &str, contents: &str) -> PathBuf {
    ...
    if !nape_test_root.exists() {
        ...
        fs::write(&nape_test_root, contents).expect(
            format!(
...
pub fn remove_subpath(subpath: &str) {
    ...
    if nape_test_root.exists() {
        if nape_test_root.is_dir() {
            fs::remove_dir_all(&nape_test_root).expect(
                format!(
            ...
            fs::remove_file(&nape_test_root).expect(
                format!("Could not remove the file '{}'.", nape_test_root.display()).as_str(),
            );
        }
    }
}
...
macro_rules! path_for {
    ($subpath:expr) => {{ test_framework_oss::nape_filesystem::generate_path_for($subpath) }};
}

/// Syntax sugar for the [`create_subpath`] function, returns the path as a [`String`]
#[macro_export]
macro_rules! create {
    ($subpath:expr) => {{ test_framework_oss::nape_filesystem::create_subpath($subpath) }};
}
...
macro_rules! create_file {
    ($subpath:expr, $contents:expr) => {{ test_framework_oss::nape_filesystem::generate_nape_testing_file($subpath, $contents) }};
    () => {};
}
...
macro_rules! create_write_only_file {
    ($subpath:expr, $contents:expr) => {{
        test_framework_oss::nape_filesystem::generate_nape_testing_file_write_only(
            $subpath, $contents,
        )
    }};
    () => {};
...
#[macro_export]
macro_rules! remove {
    ($subpath:expr) => {{ test_framework_oss::nape_filesystem::remove_subpath($subpath) }};
}
